...
public final class MathUtils
{
    ...
    public static com.badlogic.gdx.math.Matrix4 toFloatMatrix(Matrix4 matrix)
    {
        ...
        for(int i = 0; i < 4; i++)
        {
            for(int j = 0; j < 4; j++)
            {
                values[i * 4 + j] = (float) matrix.get(i, j);
            }
        }

        return new com.badlogic.gdx.math.Matrix4(values);
    }
