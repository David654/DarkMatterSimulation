...
float getSoftShadow(vec3 p, vec3 lightPos)
{
    ...
    for(int i = 0; i < uMaxSteps; i++)
    {
        ...
        if(uIDs[int(hit.y) - 1] == 1 || hit.x < EPSILON || dist > 60.0)
        {
            break;
        }
    }
...
vec3 getLight(vec3 p, vec3 rd, float id, vec3 lightPos[BODY_NUM_LIMIT], vec3 bgColor)
{
    ...
    for(int i = 0; i < uLightSourcesNum; i++)
    {
        ...
        vec3 specular = specColor * pow(clamp(dot(R, V), 0.0, 1.0), 10.0); // clamp(dot(R, V), 0.0, 1.0)

        // shadows
        float shadow = getSoftShadow(p + N * 0.02, normalize(lightPos[i]));
        ...
        finalCol += (ambient + back) * occ + (diffuse + specular * occ) * shadow; // * shadow
    }
    return finalCol;
}
