        ...
        if(uIDs[i] != 1 && uVisibilities[i] == 1)
        {
            if(uShowGrid == 1)
            {
                ...
                for(int i = 0; i < uBodyNum; i++)
                {
                    ...
                    res = fOpDifferenceRoundID(res, body, 1); // pow(uMasses[i], 1.0 / 20.0)
                }
            }
            ...
            //bodyPos.y = pModSingle1(-bodyPos.y, 20);
            //pModInterval1(bodyPos.x, length(uDimensions[i]) * 2, 0, uBodyNum - 1);

            float bump = 0;
            if(isBump)
            {
                bump = (uBump[i] == 1 ? bumpMapping(uBumpTextures[i], normalize(bodyPos), 2) : 0);
            }
